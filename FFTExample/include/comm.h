...
__STATIC_FORCEINLINE bool ringIsBufferEmpty(volatile RingBuffer_TypeDef* buffer) {
	return buffer->head == buffer->tail;
}

// Checks to see if the ring buffer is full (tail + 1 = head)
__STATIC_FORCEINLINE bool ringIsBufferFull(volatile RingBuffer_TypeDef* buffer) {
